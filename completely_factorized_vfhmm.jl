...
function updateM!(fhmm::FHMM,
    ...
    for m=1:fhmm.M
        ...
        # avoid zero division
#        println(sum(θᵐ, dims=2))
        fhmm.P[:,:,m] = fhmm.P[:,:,m] ./ (sum(θᵐ, dims=2) .+ ϵ)'
    end
...
function randθ(K::Int, M::Int, T::Int)
    return softmax(rand(K, M, T), dims=1)
end

# 
# theta is KxMxT - probability of the Mth chain being in state K at time T
#
